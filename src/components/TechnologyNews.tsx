...
import TechnologyNewsCard from "./TechnologyNewsCard";
...
const TechnologyNews: React.FC<NewsGridProps> = ({
  news,
  loading = false,
  error = null,
  onRefresh,
    ...
    <div className="">
      ...
      <div className="relative w-full max-w-7xl mx-auto overflow-hidden group">
        ...
        <div className="flex items-center mb-4 px-2 md:px-0">
          <h1 className="mr-auto font-bold text-xl">Technology</h1>
        ...
        <div
          ...
          onMouseEnter={stopAutoScroll}
          onMouseLeave={startAutoScroll}
          className="flex  gap-6 overflow-x-auto snap-x snap-mandatory scroll-smooth no-scrollbar px-2 md:px-0"
        >
          {news.map((article) => (
            <div
              key={article.id}
            ...
            >
              <TechnologyNewsCard article={article} size="large" />
            </div>
          ))}
        </div>
      </div>
